        ...
        while (true)
        {
            ...
            switch (choice)
            {
                case "T":
                    ...
                    account.InputTransactions(transactions, Acc);
                    break;
                case "I":
                    //DefineInterestRules(interestRates);
                    account.DefineInterestRules(interestRates);
                    break;
                case "P":
                    //PrintStatement(transactions, Acc, interestRates);
                    account.PrintStatement(transactions, Acc, interestRates);
                    break;
                case "Q":
...
public class BankAccount
{
    ...
    public void Withdraw(decimal amount)
    {
        ...
        {
            Transactions.Add($"Withdraw: -${amount:F2}");
            Balance -= amount;
        }
        else
        {
    ...
    public void SetInterestRate(decimal rate)
    {
        ...
        {
            InterestRate = rate;
            Console.WriteLine($"Interest rate set to {rate:F2}%");
        }
        else
        {
    ...
    public void ApplyInterest()
    {
        ...
        Balance += interest;
        Transactions.Add($"Interest applied: +${interest:F2}");
    }
    public void InputTransactions(List<Transaction> transactions, List<Account> Acc)
    {
        Console.Write("Please enter transaction details in <Date> <Account> <Type> <Amount> format (or enter blank to go back to main menu): ");
    ...
    public void DefineInterestRules(List<InterestRule> interestRates)
    {
        Console.Write("Please enter interest rules details in <Date> <RuleId> <Rate in %> format (or enter blank to go back to main menu): ");
        string interestRateRule = Console.ReadLine();
        var RuleFound = interestRates.Find(Rule => Rule.Date == interestRateRule.Split(' ')[0]);
    ...
    public void PrintTxnStatement(List<Transaction> transactions, List<Account> Acc)
    {
        if (transactions.Count == 0)
        {
            Console.WriteLine("No transactions to display.");
    ...
    public void PrintStatement(List<Transaction> transactions, List<Account> Acc, List<InterestRule> interestRateRules)
    {
        if (transactions.Count == 0)
        {
            Console.WriteLine("No transactions to display.");
        ...
        string IntTxnDate = string.Empty;
        int EOM = 0;
        calculateInterest2(CurrMonthtransactions, yyyymm, interestRateRules, ref CalcInterest, ref IntTxnDate, ref EOM);
    ...
    public void calculateInterest2(List<Transaction> transactions, string yyyymm, List<InterestRule> interestRateRules, ref decimal CalcInterest, ref string IntTxnDate, ref int EOM)
    {
        int days = 0;
